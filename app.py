...
import uuid, os, json, torch, joblib
...
def predict():
    ...
    try:
        ...
        stgcn_model.load_state_dict(torch.load('models/model_stgcn.pth', map_location='cpu'))
        stgcn_model.eval()

        with torch.no_grad():
            stgcn_output = stgcn_model(X_stgcn, A_hat)
            route_output = stgcn_output.squeeze(0).numpy()

            all_routes = {}
            for i, origin_zone in enumerate(zone_ids):
                sorted_indices = np.argsort(route_output[i])[::-1][:3]
                all_destinations = [int(zone_ids[idx]) for idx in sorted_indices]
                all_routes[f"From Zone {origin_zone}"] = all_destinations
        
        allocations_df = allocate_taxis_per_day(restructured_df, route_output, zone_ids)
        allocations_data = allocations_df.to_dict(orient='records')
        """
        results = {
            "status": "success",
            "files_processed": [f.filename for f in uploaded_files],
            "lstm_results": prediction_lstm,
            # "stgcn_results": all_routes,
...
class dataGeneratorSTGCN():
    ...
    def generateData(self, df):
        num_zones, zone_ids = self.zoneID(df)
        norm_adj = self.adjMatrix(df, self.zone_to_idx, self.num_zones)
        X, y = self.tensorData(df, self.zone_ids)
...
def allocate_taxis_per_day(restructured_df, stgcn_output, zone_ids, alpha=0.8):
    daily_unit_limit = {day: 5700 for day in range(7)}

    zone_ids = list(zone_ids)
    allocation_results = []

    for day in range(7):
        daily_df = restructured_df[restructured_df['day_of_week'] == day]

        predicted_demand = daily_df.groupby('PULocationID')['prediction'].sum().reindex(zone_ids, fill_value=0)

        inflow = pd.DataFrame(stgcn_output, columns=zone_ids, index=zone_ids).sum(axis=0)

        combined_score = alpha * predicted_demand + (1 - alpha) * inflow

        daily_total = daily_unit_limit.get(day, 5700)
        normalized_allocation = daily_total * (combined_score / combined_score.sum())

        allocation_df = pd.DataFrame({
            'zone': zone_ids,
            'predicted_demand': predicted_demand.values,
            'allocated_units': normalized_allocation.values,
            'day_of_week': day
        })

        allocation_results.append(allocation_df)

    final_allocation_df = pd.concat(allocation_results, ignore_index=True)
